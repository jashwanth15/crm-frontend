...
const audiences = ['All Customers', 'VIP', 'Premium', 'Inactive', 'Recent Buyers', 'Custom Audience'];
...
export default function EditCampaign({ campaignId, onNavigate }) {
  ...
  const [campaign, setCampaign] = useState({
    ...
    channel: 'WhatsApp',
    message: { subject: '', body: '', imageUrl: '', buttons: [] },
    status: 'Draft'
  });
  ...
  const fetchCampaign = async () => {
    try {
      ...
      if (!res.data.message.buttons) res.data.message.buttons = [];
      if (!res.data.message.messageType) res.data.message.messageType = 'Promotion';
      setCampaign(res.data);
    } catch (error) {
      console.error('Failed to load campaign', error);
      toast.error("Failed to load campaign. It may have been deleted.");
      onNavigate('campaigns');
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdate = async (status = 'Draft') => {
    setIsSubmitting(true);
    try {
      const res = await axios.put(`/api/campaigns/${campaignId}`, { ...campaign, status });
      if (status === 'Running') {
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {audiences.map(aud => {
                ...
                const isSelected = isCustomBtn
                  ? !['All Customers', 'VIP', 'Premium', 'Inactive', 'Recent Buyers'].includes(campaign.audience)
                  : campaign.audience === aud;

                return (
                  <button 
                    key={aud}
                    onClick={() => setCampaign({ ...campaign, audience: isCustomBtn ? (campaign.audience === 'Custom Audience' || !['All Customers', 'VIP', 'Premium', 'Inactive', 'Recent Buyers'].includes(campaign.audience) ? campaign.audience : 'Custom Audience') : aud })}
